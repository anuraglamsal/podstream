...
# In wtforms, all the magic that gives
...
# constant for every user.
     ...
     # section of the code.
     MiddleName = StringField('Middle Name', validators=[Length(max=50)], render_kw={"placeholder": "If any"})
     # As there might not be a middle name,
     # data is not compulsarily required. But
     # we do specify the maximum length of middle
     # name allowed.

     # As you cannot work directly with html tags of the
     # fields, wtforms provides a way for you to add
     # methods in these fields through 'render_kw'. You
     # basically provide the method keyword and its value
     # as a dictionary, and wtforms adds it in the concerned
     # tag for you. Here, I've used the 'placeholder' method
     # to add some text inside the field.
     ...
     Username = StringField('Username', validators=[DataRequired(), Length(min=5, max=20)],
                            render_kw={"placeholder": "5 to 20 characters"})
     Password = PasswordField('Password', validators=[DataRequired(), Length(min=9, max=20)],
                              render_kw={"placeholder": "9 to 20 characters"})
...
class Login(FlaskForm):
     # Here, we are creating another class that inherits
